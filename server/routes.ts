...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/files/upload", upload.single('file'), async (req, res) => {
    try {
      ...
      console.log(`📤 Starting file upload: ${req.file.originalname} (${req.file.size} bytes)`);
      ...
      try {
        ...
        console.log(`💾 File metadata stored in database (${isLargeFile ? 'large file - data in memory only' : 'small file - full data in database'})`);
      } catch (error) {
        if (error.message?.includes('too large')) {
          console.log(`📁 File too large for database, using memory-only storage`);
      ...
      console.log(`📏 Size: ${uploadedFile.size} bytes (${(uploadedFile.size / (1024 * 1024)).toFixed(1)}MB)`);
      ...
      console.log(`🔍 In tempFileMetadata: ${global.tempFileMetadata?.has(fileId) ? 'YES' : 'NO'}`);
  ...
  app.post("/api/files/:fileId/mapping", async (req, res) => {
    try {
      ...
      console.log(`📂 Available temp files:`, global.tempFileMetadata ? Array.from(global.tempFileMetadata.keys()) : 'No temp files');
      
      // Try to get file from memory first, then database
      let file = global.tempFileMetadata?.get(fileId);
      
      if (!file) {
        console.log(`⚠️ File ${fileId} not found in memory, checking database...`);
  ...
  app.post("/api/files/:fileId/process", async (req, res) => {
    try {
      ...
      if (rawData.length === 0) {
        rawData = file.data || [];
        console.log(`📋 Retrieved ${rawData.length} records from database for file ${fileId}`);
      ...
      console.log(`🏪 TOTAL Unique pickup warehouses in raw data: ${uniquePickupWarehouses.size}`);
      console.log(`📦 All pickup warehouses:`, Array.from(uniquePickupWarehouses));
      ...
      console.log(`✅ Suppliers created. Processing ${validOrders.length} orders in batches...`);
      
      // After filtering debug
      console.log(`📊 After filtering: ${validOrders.length} valid orders, ${cancelledOrders.length} cancelled`);
      console.log(`🔍 Normalized data supplier distribution:`, 
      ...
      for (let i = 0; i < validOrders.length; i += BATCH_SIZE) {
        ...
        try {
          ...
          console.log(`⚡ Batch ${batchNum}/${totalBatches} - ${batchResults.length} orders saved`);
        } catch (error) {
      ...
      console.log(`🎉 Processing complete! Total orders created: ${createdOrders.length}`)
  ...
  app.post("/api/price-entries/bulk-upload", upload.single('file'), async (req, res) => {
    try {
      ...
      console.log(`📤 Processing price list upload: ${req.file.originalname}`);
      console.log(`📋 File details: MIME: ${req.file.mimetype}, Size: ${req.file.size} bytes`);
      console.log(`🏢 Single supplier mode: ${supplierName ? 'YES (' + supplierName + ')' : 'NO (multi-supplier file expected)'}`);
      ...
      console.log(`✅ Processed ${processedCount} price entries for ${uniqueSuppliers.size} suppliers: ${Array.from(uniqueSuppliers).join(', ')}`);
  ...
  app.get("/api/export/missing-price-entries", async (req, res) => {
    ...
    try {
      ...
      console.log(`📋 Export Result: Found ${missing.length} missing price entries`);
  ...
  app.post("/api/price-entries/bulk-upload", upload.single('file'), async (req, res) => {
    try {
      ...
      console.log(`📁 Bulk Upload Started: ${req.file.originalname} (${req.file.size} bytes)`);
      ...
      if (req.file.mimetype.includes('excel') || req.file.mimetype.includes('spreadsheet')) {
        ...
        console.log(`📊 Excel parsed: ${headers.length} columns, ${dataRows.length} rows`);
      } 
      // Handle CSV files
        ...
        parsedData = { headers, dataRows };
        console.log(`📊 CSV parsed: ${headers.length} columns, ${dataRows.length} rows`);
      } else {
      ...
      for (let i = 0; i < Math.min(5, dataRows.length); i++) {
        ...
        console.log(`📊 Row ${i + 1} has ${row.length} columns:`, row.slice(0, 5), '...');
      }
      ...
      for (let i = 0; i < dataRows.length; i++) {
        try {
          ...
          console.log(`🔍 Processing row ${i + 2}: Supplier=${supplierName}, Product=${productName}, Price=${priceBeforeGST}, HSN=${hsn}`);
          ...
          console.log(`🧹 Cleaned values - Original Price: "${priceBeforeGST}", Clean Price: "${cleanPrice}", HSN: "${cleanHSN}"`);
          ...
          if (!cleanPrice || !cleanHSN || parseFloat(cleanPrice) < 0 || isNaN(parseFloat(cleanPrice))) {
            console.log(`❌ Skipping row ${i + 2}: Price=${cleanPrice}, HSN=${cleanHSN}`);
            results.skipped++;
          ...
          results.processed++;
          console.log(`✅ Successfully created price entry for ${productName}`);
